...
```{r include = FALSE}
knitr::opts_chunk$set(echo = TRUE, tidy = TRUE, highlight = FALSE)
```
...
(r.names.split.first <- sapply(r.names.split, `[`, 1))
...
(g9 <- g8 +
  scale_y_continuous(breaks = income[c(1:8, 14)], labels = income[c(1:8, 14)]))
#  scale_y_continuous(breaks = seq(0, 100, by = 25))) 
(g10 <- g9 +
  annotate("text", x = 30, y = 60, label = paste("Gini = ", round(gini, digits = 3)), size = 9, color = "red", srt = 15)) 
(g11 <- g10 +
  annotate("text", x = 80, y = 20, label = "15 Million", size = 9, color = "blue"))
(g12 <- g11 +
  theme_bw())
```
